...
class TestPoetry:
    ...
    def test_unset_restores_original_project_file(
        self, tool_installed, tmp_path, monkeypatch
    ):
        monkeypatch.chdir(tmp_path)
        config = tmp_path / "pyproject.toml"
        original = b"# comment\n[tool.poetry]\n"
    ...
    def test_unset_graceful_without_pyproject(
        self, tool_installed, tmp_path, monkeypatch
    ):
        monkeypatch.chdir(tmp_path)
        success, msg = unset_poetry_mirror()
...
class TestConda:
    ...
    def test_set_does_not_remove_existing_default_channels(
        self, tool_installed, monkeypatch
    ):
        calls = []
